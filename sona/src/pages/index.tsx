...
import { traitColorMapping } from "../../utils/Trait";
        ...
        const mappedMatches = matches.map((match: MatchI) => {
          return {
            ...
            matches_participants: [...match.matches_participants]
              .sort((a, b) => a.placement.valueOf() - b.placement.valueOf())
              .map((participant) => {
                return {
                  ...participant,
                  matches_participants_units: [
                    ...participant.matches_participants_units,
                  ].sort((a, b) => b.tier.valueOf() - a.tier.valueOf()),
                };
              })
              ...
              .filter((participant, index, self) => {
                return (
                  self.findIndex(
                    (p) => p.summoners.name === participant.summoners.name
                  ) === index
                );
              }),
          };
              ...
              <Accordion key={match.game_datetime.toString()}>
                ...
                >
                  {match.matches_participants.length &&
                    match.matches_participants.map((participant, index) => {
                      return (
                        <Accordion
                            ...
                            <Group>
                                ...
                                {participant.matches_participants_units.map(
                                  (unit) => (
                                    <>
                                      <Tooltip
                                        closeDelay={50}
                                        label={unit.characters.display_name}
                                        openDelay={50}
                                        withArrow
                                      >
                                        <Avatar
                                          key={unit.characters.display_name}
                                          radius="xs"
                                          size="lg"
                              ...
                              >
                                <Stack>
                                  <Group>
                                    <Group spacing={5}>
                                      <GiTwoCoins color="#C9AC6D" size={20} />
                                      <Text>{participant.gold_left}</Text>
                                    </Group>
                                    <Text>Lv.{participant.level}</Text>
                                  </Group>
                                  <Group>
                                    {participant.matches_participants_traits.map(
                                      (trait) => {
                                        if (!trait.tier_current) {
                                          return;
                                        }
                                        return (
                                          <Box
                                            sx={{
                                              backgroundColor: `${traitColorMapping(
                                                trait.style
                                              )}`,
                                              padding: "5px",
                                              borderRadius: "5px",
                                            ...
                                            key={trait.traits.display_name}
                                          >
                                            <Image
                                              src={`https://cdn.tft.tools/trait-icons/${trait.traits.trait_id}.svg`}
                                              width={27}
                                              height={27}
                                              alt={trait.traits.display_name}
                                            />
                                            <Text>{trait.num_units}</Text>
                                          </Box>
                                        );
                                      }
                                    )}
